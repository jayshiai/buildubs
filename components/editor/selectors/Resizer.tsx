...
} from "@/lib/editorUtils/numToMeasurement";

const Indicators = ({ bound }) => (
  <div className="pointer-events-none absolute left-0 top-0 h-full w-full">
    <span
      className={cx(
        "absolute w-2.5 h-2.5 bg-white rounded-full shadow-md border-2 border-blue-500 z-50 pointer-events-none",
        bound
          ? bound === "row"
            ? "left-1/2 top-[-5px] transform translate-x-[-50%]"
            : "top-1/2 left-[-5px] transform translate-y-[-50%]"
          : "left-[-5px] top-[-5px]"
      )}
    />
    {!bound && (
      <span className="pointer-events-none absolute right-[-5px] top-[-5px] z-50 h-2.5 w-2.5 rounded-full border-2 border-blue-500 bg-white shadow-md" />
    )}
    <span
      className={cx(
        "absolute w-2.5 h-2.5 bg-white rounded-full shadow-md border-2 border-blue-500 z-50 pointer-events-none",
        bound
          ? bound === "row"
            ? "left-1/2 bottom-[-5px] transform translate-x-[-50%]"
            : "bottom-1/2 left-[-5px] transform translate-y-[-50%]"
          : "left-[-5px] bottom-[-5px]"
      )}
    />
    {!bound && (
      <span className="pointer-events-none absolute bottom-[-5px] right-[-5px] z-50 h-2.5 w-2.5 rounded-full border-2 border-blue-500 bg-white shadow-md" />
    )}
  </div>
);
